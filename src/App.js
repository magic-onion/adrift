...
class App extends Component {
  ...
  handleSubmit = event => {
    ...
    switch (this.state.formCreates) {
      ...
      .then(r=>r.json()).then(p => {
        let users = p
        let currentUser = users.find(user => user.username === this.state.username && user.password === this.state.password)
        currentUser ? this.setState({currentUserId: currentUser.id}) : console.log('not defined')
  ...
  saveGame = () => {
    ...
    if (this.state.currentGame.event) {
      ...
      }).then(r=>r.json()).then(p=>console.log(p))
    }
    else {
  ...
  isLoggedIn = () => {
    if (this.state.currentUserId) {
      return (
        ...
        <SaveLoad saveGame={this.saveGame} loadGame={this.loadGame}/>
        </div>
      )
    }
    else { return  <UserCreate handleSubmit={this.handleSubmit} handleUsernameChange={this.handleUsernameChange} handlePasswordChange={this.handlePasswordChange} handleToggleClick={this.handleToggleClick} toggled={this.toggled} toggled2={this.toggled2} /> }
  }
